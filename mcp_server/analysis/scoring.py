...
def score_stock(technical: TechnicalIndicators, macro: MacroInfo, anchors: MacroAnchors) -> ScoreResult:
    ...
    if rsi is not None:
        if rsi < 30:
            technical_score += 15
            bullish.append("RSI is oversold, suggesting potential mean reversion")
        elif rsi > 70:
            technical_score -= 15
            bearish.append("RSI is overbought, raising pullback risk")
    ...
    if technical.last_price is not None and sma200 is not None:
        if technical.last_price > sma200:
            technical_score += 20
            bullish.append("Price is trading above the 200-day moving average")
        else:
            technical_score -= 20
            bearish.append("Price is trading below the 200-day moving average")

    if technical.trend.adx_14 is not None:
        if technical.trend.trend_signal == "UPTREND" and technical.trend.adx_14 >= 20:
            technical_score += 15
            bullish.append("ADX confirms an upward trend")
        elif technical.trend.trend_signal == "DOWNTREND" and technical.trend.adx_14 >= 20:
            technical_score -= 15
            bearish.append("ADX confirms a downward trend")
    ...
    if technical.volatility.bb_upper and technical.volatility.bb_lower and technical.last_price:
        if technical.last_price < technical.volatility.bb_lower:
            technical_score += 10
            bullish.append("Price is below the lower Bollinger Band")
        elif technical.last_price > technical.volatility.bb_upper:
            technical_score -= 10
            bearish.append("Price is above the upper Bollinger Band")
    ...
    if technical.volume.volume_ratio is not None and technical.volume.volume_ratio >= 1.5:
        if technical.trend.trend_signal == "UPTREND":
            technical_score += 10
            bullish.append("Volume confirms the upward move")
        elif technical.trend.trend_signal == "DOWNTREND":
            technical_score -= 10
            bearish.append("Elevated volume confirms selling pressure")

    regime = anchors.rates_environment.regime
    if regime == "RESTRICTIVE":
        macro_score -= 15
        risks.append("Restrictive RBA policy can pressure equity valuations")
    elif regime == "ACCOMMODATIVE":
        macro_score += 15
        bullish.append("Accommodative rates are supportive for equities")

    vix_regime = anchors.risk_sentiment.vix_regime
    if vix_regime in {"LOW", "NORMAL"}:
        macro_score += 10
        bullish.append(f"VIX regime is {vix_regime.lower()}, supporting risk appetite")
    elif vix_regime in {"ELEVATED", "EXTREME"}:
        macro_score -= 20
        risks.append(f"VIX regime is {vix_regime.lower()}, implying higher market stress")

    if anchors.china_exposure.china_signal == "POSITIVE":
        macro_score += 10
        bullish.append("China-linked macro indicators are positive")
    elif anchors.china_exposure.china_signal == "NEGATIVE":
        macro_score -= 10
        risks.append("China-linked macro indicators are negative")
    ...
    if macro.commodities.gold_usd or macro.commodities.crude_oil_usd or macro.commodities.copper_usd:
        ...
        if sum(value is not None for value in commodity_values) >= 2:
            macro_score += 5
            bullish.append("Key commodity market data is available for resource-stock context")
